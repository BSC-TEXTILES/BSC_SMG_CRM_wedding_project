...
async function autoInitializeDatabase(pool) {
  ...
  try {
    ...
    const migrations = [
      ...
      `ALTER TABLE \`FeedbackQrCode\` ADD COLUMN \`floor\` VARCHAR(100) NULL AFTER \`sectionName\``,
      `ALTER TABLE \`FeedbackQrCode\` ADD INDEX \`idx_floor\` (\`floor\`)`,
